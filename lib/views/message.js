...
Object.assign(message, view, {
  template: '<div class="message">' +
              ...
              '</video>' +
              '<img class="fallback-gif" data-hook="fallback-gif" src="" />' +
              ...
              '<div class="share-overlay" style="display:none">' +
                ...
                '<div class="slack-share">' +
                  '<a href="https://slack.com/oauth/authorize?scope=chat:write:bot,channels:read&client_id=2168708159.15814809218" data-hook="slack-button-link">' +
                  ...
                  '<div data-hook="slack-share">' +
                    '<button class="btn btn-slack btn-large" type="button" data-hook="slack-button">' +
                      '<img src="/images/slack.png" style="width:12px; opacity:1; position: relative; top:2px; left: -1px "/>' +
...
//                '</button>' +
  ...
  initialize: function({videoUrl, text, likes, room, selfie, mimeType, poster}){
    this.room = room;
    var that = this;
    ...
    if(!selfie){
      ...
      setTimeout(function(){
        if(that.el.is('.no-autoplay') || that.el.find('video').get(0).paused){
          var spinner = new Spinner({color:'#aaa', lines: 12, position: 'absolute', top: '50%', left: '50%'}).spin();
          that.el.append(spinner.el);

          setTimeout(function(){
            that.gifSource = config.gifSource + that.id + '.gif';
            that.el.find('[data-hook="fallback-gif"]').attr('src', that.gifSource);
      ...
      // Local preview of the just-recorded clip. `videoUrl` is an object (blob:)
      // URL - far more reliable on iOS than a data: URL, which iOS WebKit often
      // won't play. `mimeType` is the recorded blob's type (video/mp4 on
      // iOS/Safari, video/webm on Chrome). Point only the matching <source> at
      // the blob and drop the other, so iOS isn't handed a source whose declared
      // type doesn't match the bytes (which renders a blank square).
      var mime = (mimeType || '').toLowerCase();

      if(mime.indexOf('mp4') !== -1){
        this.mp4Source = videoUrl;
        this.el.find('source[type="video/webm"]').remove();
      } else if(mime.indexOf('webm') !== -1){
        this.webmSource = videoUrl;
        this.el.find('source[type="video/mp4"]').remove();
      } else {
        // Unknown type - let the browser probe the blob through both sources.
        this.webmSource = videoUrl;
        this.mp4Source = videoUrl;
      }

      // Only the local preview should play inline on iOS. Server messages
      // deliberately omit `playsinline` so that when iOS blocks their autoplay
      // they fall back to the animated GIF (see the !selfie branch above);
      // adding it there made iOS attempt unreliable inline playback and render
      // uploaded videos blank instead of showing the GIF.
      this.el.find('video').attr('playsinline', '');

      // Show the captured frame until (or unless) the clip plays, so the
      // message never appears as a blank square.
      if(poster){
        this.el.find('video').attr('poster', poster);
      }
    }
