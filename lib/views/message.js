...
Object.assign(message, view, {
  ...
  initialize: function({webmSource, mp4Source, text, likes, room}){
    var split = webmSource.split('/');
    this.id = split[split.length - 1].split('.')[0];

    this.room = room;

    this.el.find('source[type="video/mp4"]').attr('src', mp4Source);
    this.el.find('source[type="video/webm"]').attr('src', webmSource);
