...
//                '</button>' +
  ...
  initialize: function({videoUrl, text, likes, room, selfie}){
    ...
    if(!selfie){
      ...
      // this will be a data url, e.g. "data:video/mp4;base64,..." (iOS/Safari)
      // or "data:video/webm;base64,..." (Chrome). Point only the matching
      // <source> at it and drop the other - otherwise a source whose declared
      // type doesn't match the data (e.g. an mp4-typed source holding webm
      // bytes) causes iOS Safari to render a blank square.
      var mimeMatch = (videoUrl || '').match(/^data:(video\/[a-z0-9]+)/i);
      var mime = mimeMatch ? mimeMatch[1].toLowerCase() : '';
