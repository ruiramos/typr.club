...
Object.assign(messages, view, {
  ...
  createMessage: function({videoUrl, text, likes, mimeType, poster}, selfie, prepend){
    if(!videoUrl) return;
  ...
  prependMessage: function({videoUrl, text, likes}, silent, msg){
    ...
    if(msg && msg.uuid === app.config.uuid && this.el.find('.selfie').length === 1){
      ...
      for (var i = 0; i < this._messages.length; i++) {
        var thisMsg = this._messages[i];
        if(thisMsg.selfie){
          thisMsg.selfie = false;
          thisMsg.registerSelfie(videoUrl);
          break;
        }
      };
    } else {
      this.el.prepend(this.createMessage({
        ...
        text: text,
        likes: likes
      }, msg.uuid === app.config.uuid, true)); // @todo not always selfie - when it comes from the other person
    }
  ...
  addSelfie: function(rec, video, text, mimeType, poster){
  ...
  limitMessages: function(){
    ...
    console.log(this.getMessages(), num, this.room)
  ...
  showSharingFor: function(id){
    this._messages.forEach(function(msg){
      if(msg.id === id) msg.handleShareClick();
    })
  }
