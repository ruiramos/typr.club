...
function upload(video, videoUrl, text, room){
  ...
  var blob = video.getBlob(),
      // blob.type can be e.g. "video/webm", "video/mp4" or "video/webm;codecs=vp8"
      // - take the subtype and strip any codec params so the filename extension
      // stays clean (webm / mp4).
      extension = (blob.type.split('/')[1] || 'webm').split(';')[0],
      file = {
        name: utils.generateRandomString() + '.' + extension,
        blob: blob,
        contents: videoUrl,
      };
...
function getSlackHookWithCode(code, state){
  return $.ajax({
    ...
    type: 'GET',
    data: {
      client_id: secrets.sci,
      client_secret: secrets.scs,
      code: code,
